...
2. **Establish freshness before judging a match.** Use Drive `modifiedTime` (UTC ISO-8601) as the freshness signal—not an assumed upload/creation date. The normal resolver’s explicit source precedence remains authoritative: a selected complete handoff manifest wins; otherwise the existing canonical `Ad Copy` priority applies, with `modifiedTime` breaking ties only within the same class. During a manual copy review, compare that selected source against other recognized, complete copy documents in the resolved package. For a self-contained manifest, “complete” requires every declared AD section to independently provide primary text, headline, and uniquely declared Feed/Stories media; a standard handoff may legitimately be copy-complete while its declared media is still pending upload. Label that state **copy verified / media pending**, never launch-ready; do not select or launch it until the exact media arrives and the normal live-proof path passes. If a newer valid alternative conflicts with the selected source, do not silently override it or describe the older copy as current. Treat it as a manual launch-review block, record both filename/ID/`modifiedTime` values in the handoff, and request a source decision; the current picker has no automatic freshness-conflict flag. Audit multiple handoff manifests separately—the normal resolver does not compare all of them.
