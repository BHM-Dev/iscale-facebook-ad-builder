# Ad Builder — Project AGENTS.md

Auto-loaded by Codex at session start. Every line is actionable context. No fluff.

---

## AI Tool Routing

This repo is worked on by both **Codex** and **OpenAI Codex**. Use the right tool for the job to manage token costs.

| Task | Tool |
|------|------|
| Multi-agent builds (3+ parallel agents) | Codex |
| MCP tools — Slack DMs, browser automation, Loom review | Codex only |
| Pre-push review agent pairs | Codex |
| DB migrations + pre-push checklist | Codex |
| Simple file edits, new fields, wiring endpoints, UI tweaks | Either |
| Boilerplate, adding a column, fixing a typo | Codex preferred |

**Sync rule:** Always `git pull origin develop` before starting a session in either tool. Always push when done. The AGENTS.md and the repo are the source of truth — session history is not.

**Codex limitations:** No MCP servers (can't DM Golden, can't read Slack/Loom, can't control browser). For anything requiring those, switch to Codex.
...
2. **Establish freshness before judging a match.** Use the source file’s Drive `modifiedTime` (UTC ISO-8601)—not its parent folder’s modified time or creation/upload date. The resolver snapshots every text file in the resolved package once, then applies this deterministic rule: the newest handoff manifest is evaluated first; a complete current manifest wins unless a newer complete canonical `Ad Copy`/category source exists. Canonical names retain priority over winner/alternate and strategy/reference material, with `modifiedTime` breaking ties within that class. A strategy/reference file cannot displace a manifest. A readable partial or malformed newer Ad Copy is ignored so an older complete source remains launchable; an unreadable newer explicitly named Ad Copy fails closed and blocks the package. A no-entry planning manifest may fall back to a valid canonical source, but a live manifest with incomplete fields/media must block rather than silently fall back. For a self-contained manifest, “complete” requires every declared AD section to independently provide primary text, headline, and uniquely declared Feed/Stories media; a standard handoff may legitimately be copy-complete while its declared media is still pending upload. Label that state **copy verified / media pending**, never launch-ready; do not select or launch it until the exact media arrives and the normal live-proof path passes. When auditing, scope refreshes to known package source IDs in batches; do not launch an unbounded whole-Drive crawl from an interactive workflow.
3. **Refresh narrowly, and record the target.** For selected launch assets, identify and record each `copy_source_drive_file_id`, then refresh only those source-document IDs. The picker’s Refresh copy request does this when selected assets carry source IDs. If any selected asset lacks one, the frontend deliberately falls back to `force_full_refresh`; report that fallback and its blast radius rather than treating it as a targeted success.
4. **Keep failures isolated at every granularity.** Every per-file process, archive action, and targeted copy-source refresh must run in a DB savepoint. A malformed file, metadata issue, or DB constraint may increment the error count but cannot roll back valid work in the same batch. Retain/add this protection for every copy-refresh loop before altering it.
...
6. **Respect the current launch contract.** The current picker/launch flow requires a primary text and headline. Do not call a headline-less organic-post item complete unless the product is deliberately changed to support that format end to end; otherwise it remains a blocked launch asset.
7. **Make refresh results request-scoped.** An unrelated concurrent picker fetch must never cause a successful targeted refresh to appear empty or flag healthy Drive rows as broken.
8. **Separate browser cache from server state.** After a server-side refresh, test with a hard-reloaded/new browser session before diagnosing the picker. Cached browser state is not evidence of a production sync failure.
9. **Use the mandatory production live-proof path.** Confirm the production URL, authenticated account, and deployed commit/version first. Then verify the exact affected asset end to end:
...
Before pushing any change that touches the ad launch flow, run the pressure test agent team using the reusable agent definitions in `.Codex/agents/`:
