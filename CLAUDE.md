...
**Deploy:** Push directly to `BHM-Dev:develop` (`git push origin develop`). Deployment runs through `.github/workflows/deploy.yml`, which SSHes into the VPS, pulls `develop` and runs `docker compose up -d --build`. **Confirmed 2026-09-20: this workflow's GitHub-registered name is `Build & Push Frontend`, not `Deploy to VPS`** — the file's own `name:` field and header comment still say "Deploy to VPS" (naming drift, not yet fixed in the file), but `gh run list` / `gh workflow list` show it as `Build & Push Frontend`. Look for that name when checking run status; don't be thrown by the file's internal name mismatch. There is **no VPS-side auto-pull** — if that workflow does not run, nothing reaches production and the API keeps serving the previous commit with a 200. **Only message Golden if the push includes a DB migration.**  
...
2. The `deploy.yml` Action (GitHub-registered name **`Build & Push Frontend`**, not "Deploy to VPS" — see the Deploy note above) deploys it — it is the only path to production. **Confirm it went green** (`gh run list --limit 3`, look for `Build & Push Frontend`); do not assume a push equals a deploy. A run's `deploy` and `verify` jobs both need to succeed — `verify` polls `/api/v1/version` to confirm the VPS is actually serving the pushed commit.
...
- [x] **P&L Tracker by ad account** — shipped. `backend/app/api/v1/pnl.py` (7 endpoints, router registered in `main.py`) + `frontend/src/pages/Pnl.jsx` (736 lines) + `/pnl` nav entry live. CLAUDE.md was stale — confirmed live 2026-08-27.
- [x] **Video ad template system** — PAUSED 2026-08-27 (Steve's call). Decided it's easier to run ad creation through an **Arcads** account than build UGC generation in-house. `VideoAds.jsx` remains a wizard shell with no generation backend; the `video_finetuning_harness.py` / direct-ElevenLabs / OmniHuman R&D work stays in the repo (uncommitted local exploration, not deleted) in case this gets revisited, but no further build work is planned. Don't pick this back up without checking with Steve first.
...
- [x] **Ad-level pausing** — shipped and live-verified 2026-09-19. `60b728f`/`9601f59`/`01be50c` built it; live Chrome test (rule creation, scope-aware ad/ad-set result messaging, check run — the run correctly skipped the Meta write because the parent ad set was already paused, then the temp rule was deleted and confirmed removed) surfaced a real five-field payload-validation bug in `AutoPauseRules.jsx`, fixed and shipped in `7ad618f`, deployed clean (green Action, API 200). Ad-level pausing is now fully closed — code, review, and live verification all done.
- [x] **Meta Creative Enhancement controls (`degrees_of_freedom_spec`)** — shipped and live-verified 2026-09-19. `1f13025`/`78eee6f`/`4c50204` built the opt-in `creative_enhancements` dict + allow-list gating `AdCreativeFeaturesSpec`; live verification found `standard_enhancements` is a deprecated/rejected Meta enum value — removed from the backend allow-list and the `CreativeEnhancementsPanel.jsx` options list in `cf6af2a` (also fixed `enabledCount` to only count currently-renderable options, so a stale/removed key can't inflate the badge). Retroactive code-auditor review: no blocking/high findings — silent-drop of any lingering `standard_enhancements: true` payload is safe and consistent with existing unknown-key handling. An individual enhancement was confirmed live creating a paused test ad in Meta with the correct `enroll_status`.
- [x] **Creative Enhancements coverage for `BulkMatchImport.jsx`/`BatchPushModal.jsx`/`PushToMetaModal.jsx`** — shipped and live-verified 2026-09-19 as part of the same pass above (`308b088`, `a33342a`, plus the `cf6af2a` deprecated-flag fix which applies to all three modes via the shared `CreativeEnhancementsPanel.jsx`). No further live-verification gap open.
