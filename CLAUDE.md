...
- [ ] **Meta Creative Enhancement controls (`degrees_of_freedom_spec`)** — base MVP code shipped and live on `develop`/production (`1f13025`, `78eee6f`, `4c50204` — `create_creative()` accepts an opt-in `creative_enhancements` dict, verified 11-flag allow-list gating `AdCreativeFeaturesSpec`, panel in `AdCreativeStep.jsx`). Field/enum names were verified against the live SDK per this repo's standing rule before building (see commit history). **Not marked done** — the brief's live-Meta-verification step (confirm the payload actually reaches Meta and is honored, via a real paused test ad/Creative) hasn't been run yet. Hold alongside the other two items below pending a live-test session.
- [ ] **Creative Enhancements coverage for `BulkMatchImport.jsx`/`BatchPushModal.jsx`/`PushToMetaModal.jsx`** — code shipped and live on `develop`/production (`308b088`, `a33342a` — shared `CreativeEnhancementsPanel.jsx`, wired into all three modes, session-scoped reset discipline). Pre-push review already ran (see `a33342a` commit message: code-auditor + joel-perspective caught a cross-modal reset gap and a silent-wipe trap, both fixed). **Not marked done** — the brief's live-Meta-verification step (create a real test Creative in at least one of the three modes with an enhancement toggled on, confirm `degrees_of_freedom_spec` actually reaches Meta with the right `enroll_status`) hasn't been run yet. Hold alongside the ad-level pausing item above, pending a live-test session.
