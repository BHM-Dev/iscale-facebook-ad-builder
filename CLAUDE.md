...
### Still pending
- [x] **Copy Library performance data** — shipped `3432f20`. `get_ad_insights_map()` pulls lifetime spend + CPL from Meta during sync; Spend + CPL sortable columns (nulls-last, CPL color-coded); validated live (CPL asc surfaces cheapest winners). **Phase 2 still pending:** weight few-shot injection toward low-CPL pinned ads (`_get_library_examples()` in `copy_generation.py`, order by `cpl.asc().nulls_last()`).
