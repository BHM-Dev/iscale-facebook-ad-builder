...
def downgrade() -> None:
    ...
    # Raw SQL with IF EXISTS, not op.drop_index: a later migration
    # (n2j0k6l7m9i5_add_missing_scraped_ads_columns) drops the whole
    # content_hash column in ITS downgrade, which cascades and removes this
    # index first. Downgrades run latest-to-earliest, so by the time this
    # baseline downgrade runs, the index is already gone — op.drop_index
    # errors on that (confirmed live in the alembic-round-trip CI job,
    # every run since 2026-08-28: "index ix_scraped_ads_content_hash does
    # not exist"). The very next line drops the whole table anyway, so this
    # index would be gone either way — just needs to not error if it
    # already is.
    op.execute('DROP INDEX IF EXISTS ix_scraped_ads_content_hash')
